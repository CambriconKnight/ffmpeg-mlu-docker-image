...
#               ./load-image-ffmpeg-mlu.sh ${FULLNAME_IMAGES}
...
num=`docker images | grep -w "$MY_IMAGE" | grep -w "$VERSION" | wc -l`
echo $num
echo $NAME_IMAGE
...
docker images | grep -e "REPOSITORY" -e $MY_IMAGE
